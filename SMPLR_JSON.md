...
`SmplrJson` is the JSON descriptor that drives sample selection, region matching, and playback. It is the canonical input to `smplr.loadInstrument(json)` (plugin-side) and to `Sampler(ctx, { json })` / `sampler.reload(json)` (public API). All first-party instruments produce a `SmplrJson` internally and call `loadInstrument` with it.

The canonical TypeScript definitions live in [`src/smplr/types.ts`](./src/smplr/types.ts). This document is the reference for the JSON shape.
...
| `ampAttack` | `number` | Attack time in seconds. *(Not yet implemented — accepted but ignored.)* |
...
2. `group.<param>`
3. `json.defaults.<param>` (least specific)
4. Built-in default

Example: a region with `volume: -3` inside a group with `volume: 0` inside a JSON with `defaults: { volume: 6 }` plays at `-3` dB.
...
`SmplrRegion` also accepts every `PlaybackParams` field; region values override the group and the JSON defaults.
